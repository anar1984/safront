...
UserStory.prototype = {
    ...
    convertTextArea2HtmlAsText: function (arg) {
        ...
        var st = MapTextAreaHtml(replaceTags($(arg).text()));
    ...
    guiComponentShowProperties: function () {
        if ($('#gui_component_show_properties').is(':checked')) {
            ...
            $('#gui-desigin-component-view-main-div-id').hide(500);
...
//                that.changeRequest_hideDates();
    ...
    changeRequestForStartDate: function (el, type) {
        ...
        $('#generalview_input_history_list').html('');
...
//    },
    ...
    bindTicketToSUS: function () {
        ...
        if (!assignee) {
            Toaster.showError('Please select Assignee(s).');
            return;
        }
...
//        $('#' + divId).find('.assignSprintModal_sprintcheckbox').first()
    ...
    insertNewBacklogDesc: function () {
        ...
        this.insertNewBacklogDescDetails($('#backlogDescriptionText').val());
        $('#backlogDescriptionText').val('');
    ...
    insertNewBacklogDescDetails: function (desc, relatedId) {
        ...
        json.kv.relatedDescId = relatedId;
        var that = this;
        var data = JSON.stringify(json);
        $.ajax({
...
//                                                    )
    ...
    updateBacklogDesc: function (el, id) {
        ...
        var data = JSON.stringify(json);
        $.ajax({
            url: urlGl + "api/post/srv/serviceTmUpdatetBacklogDescription",
            type: "POST",
            data: data,
            contentType: "application/json",
            crossDomain: true,
...
//                that.loadBacklogTask();
    ...
    addTaskTypeDetails: function (filename) {
        ...
        if ($('#addTaskType_estiamtedhours').val().length === 0) {
            Toaster.showError("Estimated Hours is not entered!");
            return;
        }
        if ($('#addTaskType_assignee').val().length === 0) {
            Toaster.showError("Assignee is not selected!");
            return;
        }
...
//        }
    ...
    updateTaskType: function () {
        ...
        if ($('#updateTaskType_spenthours').val().length === 0) {
            Toaster.showError("Spent Hours is not entered!");
            return;
        }
        if (!$('#updateTaskType_status').val()) {
            Toaster.showError("Status is not selected!");
            return;
        }
...
//        var ind = getIndexOfTable(res, "inputDescListTable");
        ...
        try {
            table = this.getInputTable4StoryCard4SelectNew(res, ind, 0, '', '');
        } catch (err) {
...
<hr style="margin:0px">\n\
<button class="dropdown-item" style="color:#000000"  onclick="new UserStory().selectDataFromDbModal(this,\'send\',\'' + obj[i].id + '\')">Save To Database Relation</button>\n\
<button class="dropdown-item" style="color:#000000" onclick="new UserStory().removeSendDBRelation(this,\'' + obj[i].id + '\')">Remove Send Database Relation</button>\n\
\n\
<hr style="margin:0px">\n\
...
<button class="dropdown-item" style="color:#000000" onclick="new UserStory().removeRelationSource(this,\'' + obj[i].id + '\')">Remove API Relation</button>\n\
<hr style="margin:0px">\n\
...
//            Toaster.showError("Please enter Input name!");
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                that.addSourcedIconToUserStory(res);
                //refresh the output listinsertNewInput
...
//        this.genGUIDesign();
    ...
    insertNewInputNew: function (el, action) {
        ...
        if (action === 'OUT' && $('#newOutputCopyAsInput').is(":checked")) {
            this.insertNewInputNewBody(el, "IN");
        }
    },
    ...
    addNewApiOutput: function (el) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $('#sus-api-output-id').val(res.kv.id);
                $('#addRelatedSourceModal-new-output').val("");
                $('.create-new-output-sc-4-select').hide();
...
//                closeModal('addRelatedSUSOutputModal');
    ...
    addExistingRelation: function () {
        ...
        if ($('#sus-output-id').val()) {
            this.updateInputBySUSOutputId(inputId);
        }
    },
...
//backlogun canvas parametrleri set edilir
    ...
    insertNewInputTotal: function () {
        ...
        json.kv.cellNo = global_var.input_insert_cellno;
        json.kv.orderNo = global_var.input_insert_orderno;
        json.kv.inputType = "IN";
        json.kv.componentType = (global_var.input_insert_component) ? global_var.input_insert_component : global_var.default_us_input_component;
        var that = this;
        var data = JSON.stringify(json);
        $.ajax({
            url: urlGl + "api/post/srv/serviceTmInsertNewInput4Select",
    ...
    updateInputByAttr: function (e, type) {
        ...
        if (type == 'table') {
            ...
            if ($(e).val().trim().length == 0) {
                Toaster.showError('Input name is empty!')
                return;
            }
...
//                console.log('v=' + v);
    ...
    setGUIComponentOrderNo: function (e) {
        ...
        if (!isFloat(s)) {
            Toaster.showError()("Value must be numeric");
            return;
        }
...
//        this.updateInputOnChangeDynamic("section", $(e).val());
    ...
    updateInputOnChange: function () {
        ...
        SAInput.updateInput(id, "section", $('#us-gui-component-in-sectionn').val());
...
//            }
    ...
    setAsOutput: function () {
        ...
        var data = JSON.stringify(json);
        $.ajax({
            url: urlGl + "api/post/srv/serviceTmSetAsOutput",
            type: "POST",
            data: data,
            contentType: "application/json",
            crossDomain: true,
    ...
    setAsInput: function () {
        ...
        $.ajax({
            url: urlGl + "api/post/srv/serviceTmSetAsInput",
            type: "POST",
            data: data,
            contentType: "application/json",
            crossDomain: true,
    ...
    createTableFormModal: function () {
        ...
        var data = JSON.stringify(json);
        $.ajax({
            url: urlGl + "api/post/srv/serviceTmGetBacklogTablenameList",
            type: "POST",
            data: data,
            contentType: "application/json",
            crossDomain: true,
    ...
    createTableForm: function () {
        ...
        if (!(name)) {
            Toaster.showError("Please choose or enter table form name.");
            return;
        }

        var seletedInputs = this.getSelectedInputs();
        if (!(name)) {
            Toaster.showError("Please select input(s).");
            return;
        }
        ...
        var data = JSON.stringify(json);
        $.ajax({
            url: urlGl + "api/post/srv/serviceTmAddTableNameToInputs",
            type: "POST",
            data: data,
            contentType: "application/json",
            crossDomain: true,
...
//            }
    ...
    toggleAllInputNew: function (e) {
        if ($('#us_input_list_check_all_new').is(':checked')) {
            ...
            $('.us-input-list-item-check-box-class-new').prop("checked", false);
        }
    },
    ...
    toggleAllOutputNew: function (e) {
        if ($('#us_output_list_check_all_new').is(':checked')) {
            ...
            $('.us-output-list-item-check-box-class-new').prop("checked", false);
        }
    },
...
//        var html = this.genGUIDesignHtmlById(popupBacklogId);
        ...
        if ($(el).attr("onclick_trigger_id")) {
            ...
            var selectedFieldsByApiList = [];
            var inputIds = SACore.GetBacklogDetails(apiId, "inputIds");
            inputIds = inputIds.replace(/ /g, '');
            var inputList = inputIds.split(',');
            for (var ii in inputList) {
                var inputId4Zad = inputList[ii];
                if (SAInput.getInputDetails(inputId4Zad, 'inputType') === 'IN') {
                    selectedFieldsByApiList.push(SAInput.getInputDetails(inputId4Zad, 'inputName'))
                }
            }
    ...
    uploadStoryCardImage: function () {
        ...
        if ($('#setStoryCardUploadImageModal_file').val().trim().length > 0) {
            ...
            if (global_var.current_modal === 'loadTestCase') {
                addImageToTestCaseTrial(TestCaseTrialId, st);
            } else {
...
//        console.log('res='+JSON.stringify(res));
            ...
            for (var n = 0; n < obj.length; n++) {
                try {
                    ...
                    st += Component.GetComponentHtmlNew(comp);
                } catch (err) {
                    console.log(err)
                }
            }
...
//                    ? "<i class=\"fa fa-ticket\" style=\"color: blue;\">&nbsp;</i>"
    ...
    load: function () {
        ...
        } else if (global_var.current_modal === 'loadDashboard') {
            $('.loadDashboard').click()
        } else if (global_var.current_modal === 'loadUserManual') {
            $('.loadUserManual').click()
...
//            var obj = res.tbl[0].r;
            ...
            for (var n = 0; n < bNoList.length; n++) {
                ...
                for (var k = 0; k < usIdList.length; k++) {
                    ...
                    var obj = SACore.toJSONObject(lastId);
...
//        td2 += countLine;
    ...
    genUSLine: function (o) {
        ...
        var tr = $('<tr onclick="new UserStory().getStoryInfo(\'' + o.id + '\',this)" \n\
        ...
        $.ajax({
            ...
            success: function (res) {
                if (action === 'copy') {
                    ...
                    that.loadDetailsOnProjectSelect();
                }
...
//                Toaster.showError("error");
    ...
    notifyLabelAsChangeRequest: function () {
        ...
        if (!ids || !$("#change-mgmt-gui-design-label-list").val()) {
            Toaster.showError("Please Select Assignees!");
            return;
        }
...
//                Toaster.showError("error");
    ...
    commentEstimatedHours: function () {
        ...
        if (!$('#commentEstimatedHoursModal_estimatedhours').val()) {
            Toaster.showError('Please enter estimated hour(s).');
        }
...
//                Toaster.showError("error");
    ...
    unpinImageToStoryCard: function (obj, fname) {
        ...
        var data = JSON.stringify(json);
        $.ajax({
            url: urlGl + "api/post/srv/serviceTmUnpinImageToStoryCard",
            type: "POST",
            data: data,
            contentType: "application/json",
            crossDomain: true,
    ...
    pinImageToStoryCard: function (obj, fid, fname) {
        ...
        var data = JSON.stringify(json);
        $.ajax({
            url: urlGl + "api/post/srv/serviceTmPinImageToStoryCard",
            type: "POST",
            data: data,
            contentType: "application/json",
            crossDomain: true,
...
//        var st = this.getGUIDesignHTML4StoryCard();
        ...
        //generate input desc table
        this.setUserStoryInputsInfoOnGeneralViewDetailsPure4SelectNew(res);
...
//                Toaster.showError("error");
    ...
    toggleBacklogIsApi4StoryCard: function () {
        if (SACore.GetCurrentBaklogIsApi() === '1') {
            ...
            $("#live-prototype-show-key").hide();
        } else {
            $('#user-story-is-api').prop("checked", false);
...
//        }
    ...
    showAllGUIDetails4Select: function (backlogKeyList) {
        ...
        for (var i = 0; i < backlogKeyList.length; i++) {
            ...
            var obj = SACore.toJSONObject(backlogId);
            if (SACore.checkFilter(obj)) {
                continue;
            }
...
//        table.append(this.getTableFooter4InputTable(res.tbl[0].r[0].descriptionSourced, true));
        ...
        if (parentId.length > 0) {
            ...
            div.attr('style', 'padding:0px 0px;')
        }
        div.append(table);
        return div;
    },
...
//        var ind = getIndexOfTable(res, "inputDescListTable");
    ...
    getInputTable4StoryCard4SelectNew: function (res, ind, iteration, rowId, parentId) {
        ...
        var idxM = 1;
        for (var i = 0; i < obj.length; i++) {
            if (obj[i].inputType !== 'IN' && obj[i].inputType !== 'GUI' && obj[i].inputType !== 'TAB') {
                continue;
            }
            ...
            if (inputType === 'TAB' || inputType === 'tab') {


                try {
                    if (obj[i].fkRelatedCompId.length > 0) {
                        var usList = [];
                        try {
...
//                    .html(this.setUserStoryInputsInfoOnGeneralViewDetailsPure4Desc4SelectNew(obj[i])));
    ...
    selectDataFromDbModal: function (el, action, id) {
        if (action === 'select') {
            ...
            $('#selectFromDbModal-title').html('Save To Database Relation');
        }

        $('#selectFromDbModal-input-id').val(id);
        $('#selectFromDbModal-actiontype').val(action);
        $('#selectFromDbModal').modal('show');

        if (global_var.is_select_from_db_modal_clicked === true) {
            return;
        }
    ...
    addNewField: function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $('#selectFromDbModal-newfield').val('');
            }
        });
    },
    ...
    addNewTable: function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $('#selectFromDbModal-tableid').change();
                $('.create-new-table-4-select').hide();
                $('#selectFromDbModal-newtable').val('');
            }
        });
    },
    ...
    addNewDatabase: function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                $('#selectFromDbModal-dbid').change();
                $('.create-new-db-4-select').hide();
                $('#selectFromDbModal-newdb').val('');
            }
        });
    },
    ...
    dbFieldOnChange: function (el) {
        ...
        if (val === '-2') {
            $('.create-new-field-4-select').show();
        }
    },
    ...
    getDbFields4SelectDataModal: function (el) {
        ...
        if (val === '-2') {
            $('.create-new-table-4-select').show();
        } else if (val.length > 3) {
            this.getDbFiledList(val);
        }
    },
...
//    },
    ...
    removeSendSaveTo: function (el, id) {
        ...
        var data = JSON.stringify(json);
        $.ajax({
            url: urlGl + "api/post/srv/serviceTmRemoveSendSaveTo",
            type: "POST",
            data: data,
            contentType: "application/json",
            crossDomain: true,
    ...
    removeRelationSource: function (el, id) {
        ...
        var data = JSON.stringify(json);
        $.ajax({
            url: urlGl + "api/post/srv/serviceTmRemoveRelationSource",
            type: "POST",
            data: data,
            contentType: "application/json",
            crossDomain: true,
    ...
    removeSendDBRelation: function (el, id) {
        ...
        var data = JSON.stringify(json);
        $.ajax({
            url: urlGl + "api/post/srv/serviceTmRemoveSendDBRelation",
            type: "POST",
            data: data,
            contentType: "application/json",
            crossDomain: true,
    ...
    removeDBRelation: function (el, id) {
        ...
        var data = JSON.stringify(json);
        $.ajax({
            url: urlGl + "api/post/srv/serviceTmRemoveDBRelation",
            type: "POST",
            data: data,
            contentType: "application/json",
            crossDomain: true,
    ...
    addRelationSourcedModal: function (el, action, id) {
        ...
        if (action === 'select') {
            $('#addRelatedSourceModal-title').html('Select Data From API');
            $('#addRelatedSourceModal-outputtitle').html('Output');
            $('#addRelatedSourceModal-apititle').html("API")
        } else if (action === 'send') {
            $('#addRelatedSourceModal-title').html('Send/Save Data to Story Card/API');
            $('#addRelatedSourceModal-outputtitle').html('Input');
            $('#addRelatedSourceModal-apititle').html("Story Card/API")
    ...
    sendDataToModal: function (el, id) {
        ...
        $('#sendDataToModal-us-related-api-input-id').val(id);

        $('#sendDataToModal-us-related-apis').html('');
        if (!global_var.last_select_from_us_id) {
            $('#sendDataToModal-sus-api-output-id').html('');
        }
...
//            }
            ...
            $('#sendDataToModal-us-related-apis').append(op);
        }
...
//////
    ...
    setUserStoryInputsInfoOnGeneralViewDetailsPure4Desc4SelectNew: function (object) {
        ...
        var selectFromInput = replaceTags(SAInput.GetInputName(object.selectFromInputId));
        var selectFromBacklog = replaceTags(SACore.GetBacklogname(object.selectFromBacklogId));
        var selectFromDBField = replaceTags(SAEntity.GetFieldDetails(object.selectFromFieldId, "fieldName"));
        var selectFromDBTable = replaceTags(SAEntity.GetTableDetails(object.selectFromTableId, "tableName"));
        var selectFromDBDatabase = replaceTags(SAEntity.GetDBDetails(object.selectFromDbId, "dbName"));
        var sendToDBField = replaceTags(SAEntity.GetFieldDetails(object.sendToFieldId, "fieldName"));
        var sendToDBTable = replaceTags(SAEntity.GetTableDetails(object.sendToTableId, "tableName"));
        var sendToDBDatabase = replaceTags(SAEntity.GetDBDetails(object.sendToDbId, "dbName"));
        var sendToInput = replaceTags(SAInput.GetInputName(object.sendToInputId));
        var sendToBacklog = replaceTags(SACore.GetBacklogname(object.sendToBacklogId));
...
//        return this.closeLabel;
    ...
    insert4Task: function () {
        ...
        var ismenu = '0';
        var id = 'insertNewLabel4Task_insertLabelUseAsMenu';
        var checked = $("input[id=" + id + "]:checked").length;
        if (checked === 1) {
            ismenu = 1;
        }
        ...
        var data = JSON.stringify(json);
        $.ajax({
            url: urlGl + "api/post/srv/serviceTmInsertNewLabel4Task",
            type: "POST",
            data: data,
            contentType: "application/json",
            crossDomain: true,
            async: false,
            success: function (res) {
                new NotificationSA("").clearField('insertNewLabel4Task');
...
//        return this.closeLabel;
    ...
    addController4Insert4Task: function () {
        ...
        var startDate = ($('#insertNewSprint4Task_startdate').val());
        var endDate = ($('#insertNewSprint4Task_enddate').val());

        var f = false;
        if (sprintName.trim().length == 0) {
            new NotificationSA("Sprint name is not entered!").showInComponent('insertNewSprint4Task-sprintname');
            f = true;
        }
        if (color.trim().length == 0) {
            new NotificationSA("Sprint color is not entered!").showInComponent('insertNewSprint4Task-sprintColor');
            f = true;
        }
        if (startDate.trim().length == 0) {
            new NotificationSA("Start Date is not entered!").showInComponent('insertNewSprint4Task_startdate');
            f = true;
        }
        if (endDate.trim().length == 0) {
            new NotificationSA("End Date is not entered!").showInComponent('insertNewSprint4Task_enddate');
            f = true;
        }
        ...
        {
            new NotificationSA("End Date should be greater than Start Date!").showInComponent('insertNewSprint4Task_enddate');
            f = true;
        }
    ...
    insert4Task: function () {
        ...
        json.kv.sprintName = $('#insertNewSprint4Task-sprintname').val();
        json.kv.sprintColor = $('#insertNewSprint4Task-sprintColor').val();
        json.kv.sprintStartDate = GetConvertedDate('insertNewSprint4Task_startdate');
        json.kv.sprintEndDate = GetConvertedDate('insertNewSprint4Task_enddate');
        json.kv.fkProjectId = projectId;
        var that = this;
        var data = JSON.stringify(json);
        $.ajax({
            url: urlGl + "api/post/srv/serviceTmInsertNewSprint4Task",
            type: "POST",
            data: data,
            contentType: "application/json",
            crossDomain: true,
            async: false,
            success: function (res) {
                new NotificationSA("").clearField('insertNewSprint4Task');
...
Project.prototype = {
    ...
    insert: function () {
        ...
        if ($('#txtProjectCode').val().trim().length == 0) {
            Toaster.showError("Please enter project code");
            return;
        }
    ...
    generateTableBody4Project: function (res) {
        ...
        try {
            ...
            for (var n = 0; n < obj.length; n++) {
                ...
                st += '<td>' + replaceTags(obj[n].description) + '</td>';
                st += '<td>' + '<a href="#" data-toggle="modal" data-target="#exampleModal3"><i class="fa fa-user-circle" aria-hidden="true"></i></a>' + '</td>';
                st += '<td>' + '<a href="#" data-toggle="modal" data-target="#exampleModal2" onclick="new Project().updateproject(this,\'' + obj[n].id + '\')"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>' + '</td>';
...
//        console.log('firstActiveTab=' + firstActiveTab)
    ...
    updateMain: function () {
        ...
        if ($('#txtProjectCode').val().trim().length == 0) {
            Toaster.showError('Project Code is empty!');
            return;
        }
...
User.prototype = {
    forgetPwd: function (e) {
        ...
        if (!$('#domainName').val().trim()) {
            Toaster.showError("Please enter Domain.")
            return;
        }
        ...
        var data = JSON.stringify(json);
        $.ajax({
            url: "api/post/nasrv/serviceCrForgetPassword",
            type: "POST",
            data: data,
            async: false,
    ...
    controller4insert: function () {
        ...
        if ($.trim($('#txtUsername').val()).length == 0) {
            ...
            Toaster.showError("Username is not valid");
            f = true;
            ...
            Toaster.showError("Password is not valid");
            f = true;
            ...
            Toaster.showError("Please enter User Email");
            f = true;
        } else if (!this.isEmailValid()) {
            Toaster.showError("Email is not valid");
            f = true;
        } else if ($.trim($('#cbUserType').val()).length == 0) {
            Toaster.showError("Please enter User Type");
            f = true;
        }
        if (f)
            throw "User error";
...
//                Toaster.showError("Something went wrong. This might be caused by duplicate table.");
    ...
    changePassword: function () {
        ...
        if ($('#changePassword_newpwd').val().trim().length === 0) {
            Toaster.showError("Please enter New Password");
            return;
        }
        if ($('#changePassword_newpwdconfirm').val().trim().length === 0) {
            Toaster.showError("Please enter Confirm Old Password");
            return;
        }
        ...
        if ($('#changePassword_newpwdconfirm').val() != $('#changePassword_newpwd').val()) {
            Toaster.showError("New Password doesn't match.");
            return;
        }
    ...
    generateTableBody4User: function (res) {
        ...
        try {
            ...
            for (var n = 0; n < obj.length; n++) {
                ...
                st += '<td>' + (n + 1) + '</td>';
                st += '<td>' + '<img id="imgUserProjectInsert2" style="background-image: element;padding-right:5px;" src="' + img + '" height="34px" width="34px" class="figure-img img-fluid rounded-circle">' + obj[n].userPersonName + ' (' + replaceTags(obj[n].username) + status + ')' + t + ' </td>';
                st += '<td>' + '<a href="#" data-toggle="modal" data-target="#userInvolvedModal"><i class="fa fa-user-circle" aria-hidden="true"></i></a>' + '</td>';
                st += '<td>' + '<a href="#" data-toggle="modal" data-target="#exampleModal6" onclick="new User().beforeInsertEvent(this,\'' + obj[n].id + '\')"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>' + '</td>';
                st += '<td>' + '<a href="#" id="btnDeleteUser" onclick="new User().removeUser(\'' + obj[n].id + '\')"><i class="fa fa-trash" aria-hidden="true"></i></a>' + '</td>';
...
//                that.addNewTaskTypeToList($('#txtTaskTypeName').val(), res);
    ...
    generateTableBody4TaskType: function (res) {
        ...
        try {
            ...
            for (var n = 0; n < obj.length; n++) {
                ...
                st += '<td>' + '<a href="#" id="btnDeleteTaskType" onclick="new TaskType().deleteTaskType(this,\'' + obj[n].id + '\')"><i class="fa fa-trash"></i></a>' + '</td>';
    ...
    updateMain: function () {
        ...
        if ($('#txtTaskTypeName').val().trim().length == 0) {
            Toaster.showError('Type Name is empty!');
            return;
        }
...
//         .css("width", "100%")
//         .attr("onchange", "new UserStory().updateBacklogDesc(this,'" + id + "')")
//
//         .val($(e).attr('idesc'))
//
//     $(e).html(inp);
...
//let valuedesc;
function updateBacklogDesc(e) {
    let parentd = $(e).parent();
    let ee = $(e).parent("tr");
    let inpitt = $(e).val();
    $(e).remove();
    parentd.text(inpitt);
...
function setColoredToPDescAll(ids, color) {
    if (!ids) {
        return;
    }
    ...
    var data = JSON.stringify(json);
    $.ajax({
        url: urlGl + "api/post/srv/serviceTmColorBacklogDescription",
        type: "POST",
        data: data,
        contentType: "application/json",
        crossDomain: true,
...
function setColoredToInputDesc(el, ids, color) {
    if (!ids) {
        return;
    }
    ...
    var data = JSON.stringify(json);
    $.ajax({
        url: urlGl + "api/post/srv/serviceTmColorInputDescription",
        type: "POST",
        data: data,
        contentType: "application/json",
        crossDomain: true,
