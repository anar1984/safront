                    ...
                    .append($('<td></td>')
                            .append($('<span></span>')
                                    ...
                                    .attr('style', (inputDeleted)
                                            ...
                                            : css4newInput)
                                    .append(replaceTags(obj.param1))
                                    .append((obj.param3) ? " (Group by:" + replaceTags(obj.param3) + ")" : ""))
                            .addClass('text-left')
                            .append($('<span></span>')
                                    .addClass('c-history-input-date')
                                    .append(' (')
                                    .append(Utility.convertDate(replaceTags(obj.historyDate)))
                                    .append('  ')
                                    .append(Utility.convertTime(replaceTags(obj.historyTime)))
                                    .append(')'))
                            .append(inputDeleted)
                            .append(inputUpdate)
                            )
    ...
    changeRequestDetails_GetInputUpdateListByInputId: function (data, inputId) {
        ...
        for (var n = 0; n < o.length; n++) {
            ...
            div.append($('<div></div>')
                    ...
                    .append(replaceTags(obj.desc))
                    .append($('<i></i>').append((obj.table) ? ' -- ' + replaceTags(obj.table) : ""))
                    .append($('<span></span>')
                            .addClass('c-history-input-date')
                            .append(' (')
                            .append(Utility.convertDate(obj.date))
...
//        $('#setStoryCardUploadImageModal_file').val("");
    ...
    getGUIDesignHTMLBody: function (res, rowId, sequence) {
        ...
        var showPropertiesGeneral = (sequence.length === 1);
...
//                        .append($('<i></i>').addClass('fa fa-download'))
    ...
    generateCommentFileLine4StoryCard: function (id, name, cell, isPinned) {
        try {
            cell = (cell === 'undefined' || !cell) ? 'col-6' : cell;
    ...
    setUserStoryInforOnGeneralView4DetailsShare: function (res) {
        ...
        try {
            ...
            if (SACore.GetCurrentBaklogShowPrototype() === '1') {
                ...
                var st = this.getGUIDesignHTMLPure(res1);
...
//                var st = this.getGUIDesignHTMLPure4Share(res);
    ...
    loadStoryCardFileList4Share: function (res) {
        ...
        var resPinnedImageId = res.kv.filePinnedImageId.split(",");

        $("#storycard_filelist_4_share").html('');
        for (var i = 0; i < resName.length; i++) {
            try {
                var isPinned = (resId[i].length > 3 && resPinnedImageId.includes(resId[i].trim()));
                $("#storycard_filelist_4_share")
                        .append(this.generateCommentFileLine4StoryCard(
                                resId[i].trim(), resName[i].trim(), "col-3", isPinned));
            } catch (e) {
            }
        }
    },
...
//                alert("error");
    ...
    setUserStoryInforOnGeneralViewDetails_old: function (res) {
        var obj = res.tbl[0].r[0];
        this.hideGUIIfUserStoryIsApi(obj);
...
//                        .append($('<th></th>').attr('style', 'border-color:#5181B8').append('Table name'))
    ...
    getTableFooter4InputTable: function (res, desc, replace) {
        var tfootBig = $('<tfoot></tfoot>')

        var ds = "";
        var newDs = "";
        try {
            ds = (replace === true) ? MapTextAreaHtml(replaceTags(desc)) : desc;
        } catch (err) {
        }
        try {
            newDs = this.setUserStoryDescription4Share(res);
        } catch (err) {
        }

        try {
            if (ds.length > 0 || newDs.length > 0) {
                tfootBig.append($('<tr></tr>').attr('style', 'padding: 30px 0 30px 0;')
                        .append($('<th></th>')
...
//                  var res = SAInput.toJSONByBacklog(obj[i].param1);
            ...
            if (ct === 'sctn' || ct === 'tab') {
                ...
                try {
                    if (obj[i].param1.length > 0) {
                        var res = SAInput.toJSONByBacklog(obj[i].param1);
                        var innerTable = this.getInputTable4StoryCard(res, 0, iteration, innerRowId4Section, obj[i].id);
                        td.append('<br>');
                        td.append(innerTable);
                    }
                } catch (err) {
...
//                    .html(this.setUserStoryInputsInfoOnGeneralViewDetailsPure4Desc4SelectNew(obj[i])));
    ...
    getInputTable4StoryCard_old: function (res, ind, iteration, rowId, parentId) {
        if (iteration >= 20) {
            return;
        }
        iteration++;
        ...
        for (var i = 0; i < obj.length; i++) {
            ...
            if (obj[i].componentType === 'sctn' || obj[i].inputType === 'TAB') {
                var td = $('<td></td>')
                        .attr('style', "text-align:left;")
                        .attr('colspan', '4');
                var innerRowId4Section = (rowId === '' ? '' : rowId + '.') + (idx++);
...
//            tr.append($('<td></td>').html(obj[i].tableName));
            tr.append($('<td></td>')
                    .attr('style', 'text-align:left;')
                    .html(this.setUserStoryInputsInfoOnGeneralViewDetailsPure4Desc(obj[i]))
        ...
        table.append(this.getTableHeader4InputTable(trcount));
        try {
            table.append(this.getTableFooter4InputTable(res, res.tbl[0].r[0].descriptionSourced, true));
        } catch (er) {
        }
        table.append(tbody);
        var div = $('<div></div>').attr('id', 'sub_tbl_' + parentId)
...
//            var obj = desc.split('%IN%');
            ...
            for (var n = 0; n < obj.length; n++) {
                ...
                stln = this.getInputDescTdItem(descId, desc1, stln, color);
            }
    ...
    getInputDescTdItem: function (descId, desc1, stln, color) {
        color = (color) ? color : 'undefined';
        var colored = '<a class="colored-a" style="padding:0px 10px;border: 1px solid ' + color + ';background-color:' + color + '; border-radius: 5px;">';
        var closeColor = "</a>"
        try {
            colored = color.length > 0
                    ? colored
                    : '<a class="colored-a" style="padding:0px 10px;border: 1px solid ' + color + ';background-color:' + color + '; border-radius: 5px;">';
    ...
    setUserStoryInputsInfoOnGeneralViewDetailsPure4Desc4SelectNewsetUserStoryDescription4Share: function (res) {
        var desc = res.kv.backlogDescColored;
        var descColored = res.kv.descColored;
        var st = "";
        try {
            if (desc) {
                var obj = desc.split('%IN%');
                var coloredList = descColored.split('%IN%');
                var stln = "";
                for (var n = 0; n < obj.length; n++) {
                    var color = (coloredList[n]) ? coloredList[n] : 'undefined';
                    var colored = '<span class="colored-a" style="padding:0px 10px; margin:4px 0px;border-bottom1: 1px solid '
                            + color
                    ...
                    try {
                        colored = color.length > 0
                                ? colored
                                : '<span class="colored-a" style="border-bottom1: 1px solid ' + color
                                + ';background-color:' + color + '; border-radius: 5px;">';
                    } catch (err) {
                    }

                    stln = (obj[n])
                            ? stln + colored + '-' + replaceTags(this.fnline2Text(obj[n]))
                            + closeColor + "<br>"
                            : stln;
                }
    ...
    setUserStoryInputsInfoOnGeneralViewDetailsPure4Desc: function (obj) {
        ...
        var coloredMain = replaceTags(obj.colored);
        var st = "";
        try {
            ...
            var obj = desc.split('%IN%');
            var coloredList = coloredMain.split('%IN%');
            var stln = "";
            for (var n = 0; n < obj.length; n++) {

                var color = (coloredList[n]) ? coloredList[n] : 'undefined';
                var colored = '<span class="colored-a" style="padding:0px 10px; margin:4px 0px;border-bottom1: 1px solid ' + color
                        + ';background-color:' + color + '; border-radius: 5px;">';
                var closeColor = "</span>"
                try {
                    colored = color.length > 0
                ...
                stln = (obj[n]) ? colored + '-' + replaceTags(this.fnline2Text(obj[n])) + closeColor + "<br>" + stln : stln;
            }
