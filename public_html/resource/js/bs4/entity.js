...
//        }
    ...
    const component_type =
            `<select class='form-control' 
                    id="us-gui-component-id-field-relation" onchange="new UserStory().setGUIComponent(this)">
     ...
     $(document).on('click','#us-gui-component-cell-no-form-gen',function(){
         api_list(this);
     })
    const cell_no =
    ...
    const api_list=function(el){
                    

        callService('serviceTmgetProjectList',{},true,function(res){
            const select =`<select></select>`
            let obj = res.tbl[0].r;
            select.append(obj.map((o)=><option value='${o.id}'>${o.id}</option>));
            el.after(select);
        })
