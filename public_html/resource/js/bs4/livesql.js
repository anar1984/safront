...
function getFieldByTableId4PopupContainer(data) {
  var tableId = getCoreContainerByTable().find("select.cs-database-table-list").val();
...
function getFilterDataLine() {
  var data = {};
  getCoreContainerByTable().find(".th-header-filter-search-by-column").each(function (ev) {
...
function getCoreContainerByTable() {
...
function getDataTableRowListContainer(selectedField, data) {
  var dbname = getCoreContainerByTable()
    .find("select.cs-database-name-list")
    .find("option:selected")
    .text();
  var tablename = getCoreContainerByTable()
    .find("select.cs-database-table-list")
    .find("option:selected")
    .text();
...
function getDataTableRowCount(dbname, tablename, dataCore) {
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      getCoreContainerByTable().find(".querydatarowcount").html(res.kv.countId);
    },
  });
}
...
function getDataTableRowList(dbname, tablename, selectedField, dataCore) {
  var el = getCoreContainerByTable().find(".cs-table-database-table-zad-list");
  getCoreContainerByTable().find(".live-sql-error").text("");
  el.find("thead").html("");
  el.find("tbody").html("");
...
$(document).on("click", ".cs-sql-editor-run-btn", function (e) {
  $(this)
    .closest(".cs-modal-box")
    .find(".cs-sql-editor-run-btn")
    .toggleClass("active");
  $(this).closest(".cs-modal-box").find(".app_editor_wrapper").show(
    "slide",
    {
      direction: "up",
    },
...
$(document).on("click", ".cs-sql-editor-run-btn.active", function (e) {
  $(this).closest(".cs-modal-box").find(".app_editor_wrapper").hide(
    "slide",
    {
      direction: "up",
    },
