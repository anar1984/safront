     ...
     function dragEndHandler(e) {
        if (dragging) {
            ...
            if (global_var.current_modal === 'loadStoryCardMgmt') {
                ...
                var status = $(dragElement).closest('div.task-column').attr('status');
                var usId = $(dragElement).closest('div.task-column').attr('us-id');
                updateTask4Status(bid, aveNo, status);
                if (global_var.task_mgmt_group_by === 'userStoryTab') {
                    updateTask4ShortChangePure(usId, "fkBacklogId", bid);
                }else if (global_var.task_mgmt_group_by === 'assignee') {
                    updateTask4ShortChangePure(usId, "fkAssigneeId", bid);
                }
            } else if (global_var.current_modal === 'loadEntityDiagram') {
                 var fieldId = $(dragElement).closest('.feildSection').attr('id');
